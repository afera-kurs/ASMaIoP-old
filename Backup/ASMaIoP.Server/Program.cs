					...
					case ProtocolId.DataTransfer_MyProfile:
						{
							...
							MySqlCommand cmd = new MySqlCommand($"SELECT people_name,people_surname,people_patronymic,role_title, image_path FROM employee JOIN people ON people_ID=employee_people_ID JOIN role ON role.role_ID=employee_role_ID JOIN image ON employee_image_ID=image_ID WHERE employee_ID={EmployeeId}", database._connection);
					...
					case ProtocolId.DataTransfer_UpdateImage_CreateProfile:
						{
							...
							MySqlCommand cmd = new MySqlCommand($"INSERT INTO image(image_path) VALUES('{GeneratedPath}')", database._connection);

							cmd.ExecuteNonQuery();
							cmd = new MySqlCommand($"UPDATE employee SET employee_image_ID=LAST_INSERT_ID() WHERE employee_ID={data.sEmployeeId}", database._connection);
							cmd.ExecuteNonQuery();
